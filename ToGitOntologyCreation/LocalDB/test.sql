SELECT DISTINCT IMO_No,
         ...
         END)
         AS Flag_col,
		 (CASE  STR_TO_DATE (keel_laid, '%d-%b-%y') >= STR_TO_DATE ('01-Apr-06', '%d-%b-%y')
		 ...
		 SELECT 
		 keel_laid, (CASE keel_laid WHEN (STR_TO_DATE (keel_laid, '%d-%b-%y') >= STR_TO_DATE ('01-Apr-06', '%d-%b-%y') THEN 'YOUNG' ELSE 'OLD' END) AS age 
		 FROM ihs_tblship;